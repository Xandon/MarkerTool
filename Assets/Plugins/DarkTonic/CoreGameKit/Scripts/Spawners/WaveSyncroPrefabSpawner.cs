...
namespace DarkTonic.CoreGameKit {
    ...
    public class WaveSyncroPrefabSpawner : MonoBehaviour {
        ...
        private void StartNextEliminationWave() {
            if (GameIsOverForSpawner || _currentWave == null || !_spawnerValid || !_waveFinishedSpawning ||
                !_currentWave.IsValid) {
                return;
            }

            var hasNoneLeft = _spawnedWaveMembers.Count == 0;
            if (LevelSettings.PreviousWaveInfo.waveType == LevelSettings.WaveType.Elimination &&
                _currentWave.waveCompletePercentage < 100) {
            ...
            if (LevelSettings.PreviousWaveInfo.waveType == LevelSettings.WaveType.Elimination &&
                ...
                if (!_repeatTimer.HasValue) {
                    ...
                    _repeatWaitTime = Random.Range(_currentWave.repeatPauseMinimum.Value,
                        _currentWave.repeatPauseMaximum.Value);
